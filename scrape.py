...
def read_articles(article_set="D0601A", data_dir="DUC2006_Summarization_Documents/duc2006_docs", write=True):
    ...
    for article in articles:
        ...
        sentences = [sentence.replace("\n", " ").strip() for para in all_paras for sentence in sent_tokenize(
            para.lower())]
        sentences = [word for sentence in sentences for words in word_tokenize(sentence) for word in words if word.isalnum()]
        if len(sentences) > 0:
